...
def createTable():
	try:
		...
		connection.commit()
		print("Table \"cardtotribe\" Addition Successful!")
...
def dropTable():
	try:
		...
		delete_table_query = '''DROP TABLE cardtotribe'''

		cursor.execute(delete_table_query)
		connection.commit()
		print("Table \"cardtotribe\" Deletion Successful!")
...
def addToTable(record):
	try:
		...
		postgres_insert_query = """ INSERT INTO cardtotribe(cardid, tribeid) VALUES %s"""
		cursor.execute(postgres_insert_query, (record,))

		connection.commit()
		print("Row added to table \"cardtotribe\"")
...
def addManyToTable(recordTuple):
	try:
		...
		print(args_str)
		cursor.execute("INSERT INTO cardtotribe(cardid, tribeid) VALUES " + args_str)

		connection.commit()
		print("Multiple rows added to \"cardtotribe\"")
...
def deleteFromTable(recordId):
	try:
		...
		postgres_delete_query = """ Delete from cardtotribe where id = %s"""
		cursor.execute(postgres_delete_query, (recordId, ))
		connection.commit()
		print("Row deleted from \"cardtotribe\"")
...
def pullFromTable(column, identifier):
	try:
		...
		postgres_pull_query = """ SELECT * from cardtotribe where id = %s"""
		cursor.execute(postgres_delete_query, (recordId, ))
		results = cursor.fetchall()
		print("Results from \"cardtotribe\" where id = %s" % (recordId))
		for row in results:
			for col in row:
				print(col, end='')
			print('')
