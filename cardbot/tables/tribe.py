...
def createTable():
	try:
		...
		connection.commit()
		print("Table \"tribe\" Addition Successful!")
...
def dropTable():
	try:
		...
		delete_table_query = '''DROP TABLE tribe'''

		cursor.execute(delete_table_query)
		connection.commit()
		print("Table \"tribe\" Deletion Successful!")
...
def addToTable(record):
	try:
		...
		connection.commit()
		print("Row added to table \"tribe\"")
...
def deleteFromTable(recordId):
	try:
		...
		postgres_delete_query = """ Delete from tribe where id = %s"""
		cursor.execute(postgres_delete_query, (recordId, ))
		connection.commit()
		print("Row deleted from \"tribe\"")
...
def pullFromTable(recordId):
	try:
		...
		postgres_pull_query = """ SELECT * from tribe where id = %s"""
		cursor.execute(postgres_delete_query, (recordId, ))
		results = cursor.fetchall()
		print("Results from \"tribe\" where id = %s" % (recordId))
		for row in results:
			for col in row:
				print(col, end='')
			print('')
